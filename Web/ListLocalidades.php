...
<div class="panel panel-info center-block" style="max-width:512px">
  <div class="panel-heading">
    <div class="panel-title">
      <span class="cond-title">Localidades</span> 
    </div>
  </div>
  ...
  <div class="panel-footer">
    <div class="btn-group sel-orden" role="group">
      <button type="button" id="btnNew" class="btn btn-default" onClick="OnNewCiudad()">Nueva</button>
    </div>
...
<div id="dlgCiudad" title="Datos de la ciudad">
  <div class="input-group"><span class="input-group-addon">Nombre:</span>
    <input type="text" id="Nombre" class="form-control" placeholder="Escriba nombre de la ciudad">
  </div>
</div>

<div id="dlgNew" title="Datos de la ciudad">
  <div class="input-group"><span class="input-group-addon">Nombre:</span>
    <input type="text" id="NombreNew" class="form-control" placeholder="Escriba nombre de la ciudad">
  </div>
</div>
...
function DoQuery( page )
  ...
  var sQuery = "SELECT IdCiudad, Name FROM ciudad ";
...
function OnClickLista( IdCiudad )
  {
	$("#dlgNew").dialog( "close" );
  
  NowItem = IdCiudad;
  
  var sId = "#Item" + IdCiudad;
  var sName = $(sId).text();
  
  $("#Nombre").val( sName );
  
	$("#dlgCiudad").dialog( "open" );
  }

$("#dlgCiudad").dialog({
	autoOpen: false,
	width: 350,
	buttons: [ {text:"Borrar", click:function() {BorrarCiudad();}}, {text:"Modificar", click:function() {ModificarCiudad(1);}}	]
});

function BorrarCiudad()
  {
	$("#dlgCiudad").dialog( "close" );
  
  jQuery.post( "Handles/DoAction.php", "Query=DELETE FROM ciudad WHERE IdCiudad="+ NowItem)
  .success(function(data) 
            {
...
function ModificarCiudad()
  ...
  jQuery.post( "Handles/DoAction.php", "Query=UPDATE ciudad SET name='"+$("#Nombre").val()+"' WHERE IdCiudad = " + NowItem)
  .success(function(data) 
            {
            if( data=="OK" )  DoQuery(-1);  
...
$("#dlgNew").dialog({
	autoOpen: false,
	width: 350,
	buttons: [ {text:"Adicionar", click:function() {AddCiudad();}} ]
});
...
function AddCiudad()
  ...
  jQuery.post( "Handles/DoAction.php", "Query=INSERT INTO ciudad (Name) VALUES ('"+ $("#NombreNew").val() +"')" )
  .success(function(data) 
            {
            if( data=="OK" )  DoQuery(-1);  
            else              alert("Error al adicionar la localidad:\r\n" + data ); 
